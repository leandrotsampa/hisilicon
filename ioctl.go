...
func Ioctl(fd, op uintptr, arg interface{}) error {
	...
	if arg == nil {
		_, _, err = syscall.Syscall(syscall.SYS_IOCTL, fd, op, 0)
	} else {
		vo := reflect.ValueOf(arg)
		if vo.Kind() != reflect.Ptr {
			panic("Invalid argument: arg must be a pointer.")
		}
		_, _, err = syscall.Syscall(syscall.SYS_IOCTL, fd, op, vo.Pointer())
	}
